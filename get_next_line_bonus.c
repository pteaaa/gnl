...
/*   Updated: 2022/02/03 20:15:39 by tperes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>
#include <stdio.h>
...
{
	int			error;
	static char	start[1024][BUFFER_SIZE + 1];

	if (start[fd][0] != '\0' && *res == '\0')
		res = get_start(start[fd], res, &nl);
	if (!res)
		return (NULL);
	while (nl == -1)
	{
